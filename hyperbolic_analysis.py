...
class InterpSettings:
    def __init__(
        self,
        lang1: enums.LanguageFamily,
        lang2: enums.LanguageFamily,
...
def _get_connected_component_bfs(
    ...
    """Gets the connected component of a graph which contains the given node.
...
def construct_knn_graph(
    ...
    if interp_settings is not None:
        arrs += covariance_analysis.interspeaker_interp(
            langfam=language_family,
...
def construct_RFA_matrix(
    ...
    Klimovskaia et al.'s ``Poincare maps for analyzing complex hierarchies in single-cell
    data'' (2020) via Chebotarev and Shamis's "The Matrix-Forest Theorem and
    Measuring Relations in Small Social Groups" (2006).
...
def get_embeddings(
    ...
    Args:
        graph (KNNNode): The kNN graph for which the Poincare embeddings are to be
                         learned.
        language_family (enums.LanguageFamily): The language family for which the embeddings
                                                are calculated.
...
def get_poincare_centroid_lang(
    embedding_dict: Dict,
    lang: enums.LanguageFamily,
) -> np.array:
    """Gets the centroids for a given language from a given embedding dictionary.

    Args:
        embedding_dict (Dict): An embedding dictionary as generated by
                                    hyperbolic_analysis.py > get_embeddings.
        lang (enums.LanguageFamily): The language for which to calculate centroids.
...
def get_poincare_centroids(
    ...
    Args:
        embedding_dict (Dict): An embedding dictionary as generated by
                                    hyperbolic_analysis.py > get_embeddings.
        language_family (enums.LanguageFamily): The language family for which the Poincare
                                          centroids are calculated

    Returns:
        Dict: A dictionary of centroids.
    """
    if language_family == enums.RomanceLanguages:
        centroid_dict = {
            "FR": None,
        ...
        # AE_centroid_x, AE_centroid_y = get_poincare_centroid_lang(
        #     embedding_dict, enums.GermanicLanguages.AMERICAN_ENGLISH
        # )
        BE_centroid_x, BE_centroid_y = get_poincare_centroid_lang(
            embedding_dict, enums.GermanicLanguages.BRITISH_AUSTRALIAN_ENGLISH
        )
        DT_centroid_x, DT_centroid_y = get_poincare_centroid_lang(
            embedding_dict, enums.GermanicLanguages.DUTCH
        )
        GR_centroid_x, GR_centroid_y = get_poincare_centroid_lang(
            embedding_dict, enums.GermanicLanguages.GERMAN
        )
        SW_centroid_x, SW_centroid_y = get_poincare_centroid_lang(
            embedding_dict, enums.GermanicLanguages.SWEDISH
        )

        # centroid_dict["AE"] = np.array([AE_centroid_x, AE_centroid_y])
        centroid_dict["BE"] = np.array([BE_centroid_x, BE_centroid_y])
        centroid_dict["DT"] = np.array([DT_centroid_x, DT_centroid_y])
        centroid_dict["GR"] = np.array([GR_centroid_x, GR_centroid_y])
        centroid_dict["SW"] = np.array([SW_centroid_x, SW_centroid_y])
...
def rotate_disk(
    centroids: Dict,
    disk: Dict,
    lang_family: enums.LanguageFamily,
) -> Dict:
    """Rotates disk such that

    Args:
        centroids (Dict): A dictionary of per-language embeddings in the disk.
        disk (Dict): Another dictionary of per-language embeddings in the disk,
                          this one to be rotated with respect to centroids to minimize
                          the distance between their like points.
        lang_family (enums.LanguageFamily): The language family for which disks are being
                                            rotated

    Returns:
        Dict: A dictionary containing the points of the rotated disk.
    """

    if lang_family == enums.RomanceLanguages:
    ...
    elif lang_family == enums.GermanicLanguages:
        langs = utils.germanic_langs

        obj_r = {
            # "AE": euclidean(disk["AE"], np.zeros(2)),
            "BE": euclidean(disk["BE"], np.zeros(2)),
            "DT": euclidean(disk["DT"], np.zeros(2)),
            "GR": euclidean(disk["GR"], np.zeros(2)),
            "SW": euclidean(disk["SW"], np.zeros(2)),
        }

        obj_prior_theta = {
            # "AE": utils._recover_theta(disk["AE"]),
            "BE": utils._recover_theta(disk["BE"]),
            "DT": utils._recover_theta(disk["DT"]),
            "GR": utils._recover_theta(disk["GR"]),
            "SW": utils._recover_theta(disk["SW"]),
        }
...
def align_all_digit_disks(
    ...
    Args:
        ...
        language (enums.LanguageFamily): The language family for which digit disks
                                         are aligned.
    ...
    graphs = {digit: construct_knn_graph(language_family=language_family, digit=digit, k=k,) for digit in digits}

    embeddings = {
        digit: get_embeddings(
            graph=graphs[digit],
            language_family=language_family,
        )
        for digit in digits
    ...
    all_points = {lang: [] for lang in langs}
