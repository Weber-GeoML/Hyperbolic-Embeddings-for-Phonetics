...
def _create_mean_spectrograms():
    ...
    with connect(
        ...
        for dgt in range(1, 11):
            for lang in [l.value for l in enums.GermanicLanguages]:
                mean_spct = np.zeros(
                    (
                ...
                counter = 0
                if lang == enums.GermanicLanguages.AMERICAN_ENGLISH.value:
                    for spkr in [s.value for s in enums.AmericanEnglishSpeakers]:
                        spct = np.array(
                            tas_cur.execute(
                        ...
                        if list(spct) != []:
                            mean_spct += spct
                            counter += 1
                elif lang == enums.GermanicLanguages.BRITISH_AUSTRALIAN_ENGLISH.value:
                    for spkr in [
                        s.value for s in enums.BritishAustralianEnglishSpeakers
                    ]:
                        spct = np.array(
                            tas_cur.execute(
                                f"SELECT * FROM {lang}{spkr}_word{dgt}"
                        ...
                        if list(spct) != []:
                            mean_spct += spct
                            counter += 1
                elif lang == enums.GermanicLanguages.DUTCH.value:
                    for spkr in [s.value for s in enums.DutchSpeakers]:
                        spct = np.array(
                            tas_cur.execute(
                                f"SELECT * FROM {lang}{spkr}_word{dgt}"
                            ).fetchall()
                        ...
                        if list(spct) != []:
                            mean_spct += spct
                            counter += 1
                elif lang == enums.GermanicLanguages.GERMAN.value:
                    for spkr in [s.value for s in enums.GermanSpeakers]:
                        spct = np.array(
                            tas_cur.execute(
                                f"SELECT * FROM {lang}{spkr}_word{dgt}"
                            ).fetchall()
                        ...
                        if list(spct) != []:
                            mean_spct += spct
                            counter += 1
                elif lang == enums.GermanicLanguages.SWEDISH.value:
                    for spkr in [s.value for s in enums.SwedishSpeakers]:
                        spct = np.array(
                            tas_cur.execute(
                                f"SELECT * FROM {lang}{spkr}_word{dgt}"
                            ).fetchall()
    ...
    with connect(f"{enums._DBType.RESIDUAL_SPECTROGRAM.value}.db") as rs_c:
        ...
        for lang in [l.value for l in enums.GermanicLanguages]:
            ...
            if lang == enums.GermanicLanguages.AMERICAN_ENGLISH.value:
                ...
                for spkr in [s.value for s in enums.BritishAustralianEnglishSpeakers]:
                    for dgt in range(1, 11):
                        spct = np.array(
                            rs_cur.execute(
                                f"SELECT * FROM {lang}{spkr}_word{dgt}"
...
def _calculate_covariance(
    language: enums.GermanicLanguages,
    cov_type="",
) -> None:
    """Calculates a covariance structure.

    Args:
        language (enums.GermanicLanguages): The language for which to calculate
    ...
    """
    # type checks
    if type(language) != enums.GermanicLanguages:
        raise Exception(f"language: {language} is not a valid GermanicLanguages enum")

    if language == enums.GermanicLanguages.AMERICAN_ENGLISH:
        speakers = enums.AmericanEnglishSpeakers
    elif language == enums.GermanicLanguages.BRITISH_AUSTRALIAN_ENGLISH:
        speakers = enums.BritishAustralianEnglishSpeakers
    elif language == enums.GermanicLanguages.DUTCH:
        speakers = enums.DutchSpeakers
    elif language == enums.GermanicLanguages.GERMAN:
        speakers = enums.GermanSpeakers
    elif language == enums.GermanicLanguages.SWEDISH:
        speakers = enums.SwedishSpeakers
...
def _calculate_covariances() -> None:
    ...
    Returns:
        None.
    """
    for lang in [l for l in enums.GermanicLanguages]:
